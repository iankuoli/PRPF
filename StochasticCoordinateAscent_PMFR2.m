    ...
    while IsConverge == false && i < MaxItr
        ...
        if approx_dist == 3
            ...
            for u = 1:usr_idx_len
                ...
                sort_max_matX = 0;
                sort_max_idx = inf;
                sort_max_idxs = -1 * ones(1,length(i_X));
                for sort_idx = 1:length(i_X)
                    if sort_max_matX < vec_matX_u(i_X(sort_idx))
                        sort_max_idxs(sort_max_idxs(1:(sort_idx-1))==-1) = sort_max_idx;
                        sort_max_matX = vec_matX_u(i_X(sort_idx));
                        sort_max_idx = i_pred(sort_idx);
                    elseif sort_max_matX == vec_matX_u(i_X(sort_idx))
                        if vec_predict_X_u(sort_max_idx) < v_pred(sort_idx);
                           sort_max_idx = i_pred(sort_idx);
                        end
                    end
                end
                sort_max_idxs(sort_max_idxs(1:(sort_idx))==-1) = sort_max_idx;
                
                sort_max_idxs2 = -1 * ones(1,length(i_X));
                sort_max_idxs = [sort_max_idxs(1,1) sort_max_idxs];
                for iii = 1:length(i_X)
                    idxx = length(i_X) - iii + 1;
                    if sort_max_idxs(idxx+1) ~= sort_max_idxs(idxx)
                        sort_max_idxs2(sort_max_idxs2==-1) = sort_max_idxs(idxx);
                        sort_max_idxs2(1:(idxx-1)) = -1;
                    end
                end
                sort_max_idxs2(sort_max_idxs2==-1) = sort_max_idxs(1);
                ...
                predict_X_diff_xui_xuj_f = delta * bsxfun(@plus, -vec_s', vec_predict_X_u);
                exp_diff_predict_xij_f = exp(predict_X_diff_xui_xuj_f);
                ...
                predict_X_diff_xui_xuj_g = delta * bsxfun(@plus, vec_s', -vec_predict_X_u);
                exp_diff_predict_xij_g = exp(predict_X_diff_xui_xuj_g);
                ...
                vec_lambda(W_toolarge_mask,:) = -repmat((h_function_s(W_toolarge_mask) + 1) ./ l_function_s(W_toolarge_mask), 1, 2);
