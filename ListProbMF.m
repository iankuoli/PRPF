function [matClusteringRes] = ListProbMF(k, prior, ini_scale, usr_batch_size, lr, lambda, topK, test_size, test_step, ini, MaxItr, check_step)
    %
    ...
    while IsConverge == false && i < MaxItr
        ...
        lambda_theta = lambda;
        lambda_beta = lambda;
        matLr_theta = lambda_theta * matTheta(usr_idx,:);
        matLr_beta = lambda_beta * matBeta(itm_idx,:);
        
        predict_X = (matTheta(usr_idx,:) * matBeta(itm_idx,:)') .* (matX(usr_idx, itm_idx)>0);
        exp_predX = spfun(@(x) exp(x), predict_X) .* (matX(usr_idx, itm_idx)>0);
        exp_matX = spfun(@(x) exp(x), matX(usr_idx, itm_idx));
        
        norm_exp_predX = sum(exp_predX, 2);
        norm_exp_matX = sum(exp_matX, 2);
        
        for u = 1:usr_idx_len
            u_idx = usr_idx(u);
            if nnz(matX(u_idx, itm_idx)) < 0
                continue;
            end
            ...
            % Update matTheta
            %
            matLr_theta(u,:) = matLr_theta(u,:) + (exp_predX(u, :) ./ norm_exp_predX(u) - exp_matX(u,:) ./ norm_exp_matX(u)) * matBeta(itm_idx, :);             
            
            %
            % Update matBeta
            %
            matLr_beta(itm_idx, :) = matLr_beta(itm_idx, :) + (exp_predX(u, :) ./ norm_exp_predX(u) - exp_matX(u,:) ./ norm_exp_matX(u))' * matTheta(u, :);
        end
        
%         %
%         % Update matBeta
%         %
%         matLr_beta2 = lambda_beta * matBeta(itm_idx,:);
%         for j = 1:itm_idx_len
%             matLr_beta2(j,:) = matLr_beta2(j,:) + (exp_predX(:, j) ./ norm_exp_predX - exp_matX(:,j) ./ norm_exp_matX)' * matTheta(usr_idx, :);
%         end
        ...
        % Plackett-Luce model based permutation probability
        obj_func = 0;
        predict_X = (matTheta(usr_idx,:) * matBeta(itm_idx,:)') .* (matX(usr_idx, itm_idx)>0);
        exp_predX = spfun(@(x) 1./(1+exp(-x)), predict_X) .* (matX(usr_idx, itm_idx)>0);
        for u = 1:usr_idx_len
            u_idx = usr_idx(u);
            if nnz(matX(u_idx, itm_idx)) < 0
                continue;
            ...
            for j = 1:length(idx_sort)
                tmp = tmp + log(exp_predX(u, js(idx_sort(j))) / sum(exp_predX(u, js(idx_sort(j:end))),2));
            end
            obj_func = obj_func + tmp;
        end
