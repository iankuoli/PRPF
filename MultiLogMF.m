function [matClusteringRes] = MultiLogMF(k, prior, ini_scale, usr_batch_size, gamma, lambda, alpha, topK, test_size, test_step, ini, MaxItr, check_step)
    %
    ...
    while IsConverge == false && i < MaxItr
        ...
        matTmp = bsxfun(@times, matTmp, vecNu(usr_idx)) .* (matX(usr_idx, itm_idx)>0);
        matTmp = matX(usr_idx, itm_idx) - matTmp;
