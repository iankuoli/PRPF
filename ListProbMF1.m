function [matClusteringRes] = ListProbMF1(k, prior, ini_scale, usr_batch_size, kappa, lambda, topK, test_size, test_step, ini, MaxItr, check_step)
    %
    ...
    while IsConverge == false && i < MaxItr
        ...
        for u = 1:usr_idx_len
            ...
            for j = 1:length(vec_matX_u)
                tmp_theta = tmp_theta + ...
                            (diff_sigma_predX(u, js(idx_sort(j:end))) * matBeta(itm_idx(js(idx_sort(j:end))),:)) ./ sum(sigma_predX(u, js(idx_sort(j:end))), 2) - ...
                            diff_sigma_predX(u, js(idx_sort(j))) ./ sigma_predX(u, js(idx_sort(j))) * matBeta(itm_idx(js(idx_sort(j))),:);
            end
            ...
            % Update matBeta
            %
            tmp_beta = zeros(1, length(vec_matX_u));
            for l = 1:length(vec_matX_u)
                tmp = 1 ./ sum(sigma_predX(u, js(idx_sort(l:end))), 2);
                tmp_beta(l:end) = tmp_beta(l:end) + tmp;              
            end
            tmp_beta1 = ((tmp_beta - 1 ./ sigma_predX(u, js(idx_sort))) .* diff_sigma_predX(u, js(idx_sort)))' * matTheta(u,:);
            matLr_beta(itm_idx(js(idx_sort)),:) = matLr_beta(itm_idx(js(idx_sort)),:) + tmp_beta1;
        end
        ...
        for u = 1:usr_idx_len
            ...
            for j = 1:length(idx_sort)
                tmp = tmp + log(sigma_predX(u, js(idx_sort(j))) / sum(sigma_predX(u, js(idx_sort(j:end))),2));
            end
